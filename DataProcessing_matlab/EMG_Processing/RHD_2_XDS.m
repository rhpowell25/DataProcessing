...
XDS_Path = 'C:\Users\rhpow\Documents\Grad School\Pop\20210902\XDS\Sorted\PG\';
...
for xx = 1:length(XDS_Files)
    ...
    % Load the xds file
    load_File_Name = XDS_Files(xx).name;
    disp(load_File_Name);
    load(strcat(XDS_Path, load_File_Name), 'xds');

    File_Name = erase(load_File_Name, '.mat');

    % Make sure the meta file name and file name match
    if ~isequal(xds.meta.rawFileName, File_Name)
    ...
    %[amplifier_data, amplifier_channels, t_amplifier, rhd_samp_rate, board_dig_in_data] = Concat_RHD_files(File_Name);
